...
[Elfa AI](https://www.elfa.ai/) monitors X (Twitter) and Telegram in real time. Every 2 hours, Tredie's backend polls these Elfa endpoints:
...
| `/v2/data/top-mentions` | Per-asset mindshare % (oracle refresh, every 15 min) |
...
3. Creates on-chain: `Market` + SPL `Mint` + `MindshareOracle` + `Vault` in a single transaction
4. Market is live in **under 10 seconds** from detection
