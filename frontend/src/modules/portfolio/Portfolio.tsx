...
function formatRelativeTime(blockTime: string): string {
  const now = Date.now();
  const ts = Number(blockTime) * 1000;
  const diff = now - ts;
...
export default function Portfolio() {
  ...
  const linkedSolanaAddress = (user?.linkedAccounts as any[])?.find(
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-white/[0.03] border border-white/[0.07] rounded-2xl p-8 grid grid-cols-2 md:grid-cols-5 gap-6">
          {[
            {
              label: "Realized PnL",
              value: isLoading
                ? "—"
                : formatSol(stats?.realized_pnl_lamports ?? 0),
              unit: "SOL",
            ...
            },
            {
              ...
              value: isLoading
                ? "—"
                : formatSol(stats?.total_volume_lamports ?? 0),
              unit: "SOL",
            },
            {
              ...
              value: isLoading
                ? "—"
                : lamportsToSol(
                    stats?.avg_profit_per_trade_lamports ?? 0,
                  ).toFixed(4),
              unit: "SOL",
            },
            {
              ...
              value: isLoading
                ? "—"
                : `${((stats?.win_rate_bps ?? 0) / 100).toFixed(1)}%`,
            },
            {
              label: "Trades",
              value: isLoading ? "—" : String(stats?.total_trades ?? 0),
            },
          ].map(({ label, value, unit, colored }) => (
            <div key={label} className="flex flex-col gap-1.5">
              <span className="text-white/40 text-xs">{label}</span>
              <span
                className={`font-mono text-sm ${colored !== null && colored !== undefined ? pnlColor(colored) : ""}`}
              >
                {value}
                {unit && value !== "—" && (
        ...
        <div className="flex items-center gap-8 border-b border-white/[0.07]">
          {(["positions", "activity"] as const).map((tab) => (
            <button
              ...
              className={`pb-4 text-sm font-medium relative transition-colors capitalize cursor-pointer group ${activeTab === tab ? "text-white" : "text-white/40 hover:text-white/70"}`}
            >
              <div className="h-5 overflow-hidden">
                <div className="flex flex-col group-hover:-translate-y-1/2 transition-transform duration-300 ease-out">
                  <span className="block leading-5">{tab}</span>
                  <span className="block leading-5" aria-hidden="true">{tab}</span>
                </div>
              </div>
              {activeTab === tab && (
            ...
            {isLoading ? (
              <div className="space-y-3">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-16 bg-white/[0.03] border border-white/[0.05] rounded-xl animate-shimmer"
                  />
                ))}
              </div>
            ) : isError ? (
              ...
              <div className="flex flex-col gap-2">
                {positions.map((pos) => (
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/tokens/${encodeURIComponent(pos.identifier)}`,
                      )
                    }
                    className=" text-left bg-white/[0.03] border border-white/[0.06] rounded-xl px-5 py-4 hover:bg-white/[0.06] hover:border-white/[0.12] transition-colors cursor-pointer"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-white font-medium text-sm line-clamp-2 leading-snug">
                      ...
                      <div className="flex items-center gap-4 sm:gap-8 text-right flex-shrink-0">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="font-mono text-sm">
                            {formatSol(pos.held_value_lamports)}{" "}
                        ...
                        <div className="hidden sm:flex flex-col gap-0.5">
                          ...
                          <span
                            className={`font-mono text-sm ${pnlColor(pos.unrealized_pnl_lamports)}`}
                          >
                            {Number(pos.unrealized_pnl_lamports) >= 0
                              ? "+"
                              : ""}
                            {formatSol(pos.unrealized_pnl_lamports)}
                          </span>
                        </div>
                        <div className="hidden sm:flex flex-col gap-0.5">
                          <span className="text-white/30 text-[11px]">
                          ...
                          <span
                            className={`font-mono text-sm ${pnlColor(pos.realized_pnl_lamports)}`}
                          >
                            {Number(pos.realized_pnl_lamports) >= 0 ? "+" : ""}
                            {formatSol(pos.realized_pnl_lamports)}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="sm:hidden mt-3 pt-3 border-t border-white/[0.05] flex items-center gap-6">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span
                          className={`font-mono text-xs ${pnlColor(pos.unrealized_pnl_lamports)}`}
                        >
                          {Number(pos.unrealized_pnl_lamports) >= 0 ? "+" : ""}
                          {formatSol(pos.unrealized_pnl_lamports)}
                        </span>
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span
                          className={`font-mono text-xs ${pnlColor(pos.realized_pnl_lamports)}`}
                        >
                          {Number(pos.realized_pnl_lamports) >= 0 ? "+" : ""}
                          {formatSol(pos.realized_pnl_lamports)}
                        </span>
          ...
          (isLoading ? (
            <div className="space-y-3">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="h-14 bg-white/[0.03] border border-white/[0.05] rounded-xl animate-shimmer"
                />
              ))}
            </div>
          ) : activity.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {activity.map((tx) => {
                const isBuy = tx.side === 0;
                const ticker = tx.identifier.includes(":")
                  ? tx.identifier.split(":")[1]
                  : tx.identifier;
                const solAmt = formatSol(tx.sol_amount);
                const tokenAmt = `${formatTokenAmount(tx.token_amount)} ${ticker}`;
                const gave = isBuy ? `${solAmt} SOL` : tokenAmt;
                const got = isBuy ? tokenAmt : `${solAmt} SOL`;
                return (
                  <div
                    key={tx.signature}
                    className="flex gap-3 bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3"
                  >
                    <span
                      className={`text-[11px] font-bold px-2 py-0.5 rounded flex-shrink-0 h-fit mt-0.5 ${isBuy ? "bg-[#22C55E]/10 text-[#22C55E]" : "bg-[#EF4444]/10 text-[#EF4444]"}`}
                    >
                      {isBuy ? "BUY" : "SELL"}
                    </span>
                    ...
                    <div className="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      <a
                        ...
                        className="text-white/20 hover:text-white/60 active:text-white/80 transition-colors p-1 -mr-1"
